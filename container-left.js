...
const zoom = d3.zoom()
  .scaleExtent([1, Math.max(numColumns, numRows)])
  .on("zoom", zoomed);

function zoomed() {
  const transform = d3.event.transform;
  const maxPanX = ((transform.k - 1) * containerWidth) / 2;
  const maxPanY = ((transform.k - 1) * containerHeight) / 2;

  transform.x = Math.min(Math.max(transform.x, -maxPanX), maxPanX); // Restrict horizontal panning within the grid's boundaries
  transform.y = Math.min(Math.max(transform.y, -maxPanY), maxPanY); // Restrict vertical panning within the grid's boundaries
  ...
  gridContainer.attr("transform", transform);
    ...
    function updateGrid(data) {
        ...
        .attr('y', (d, i) => Math.floor(i / numColumns) * gridItemHeight)
            ...
            .on('mouseover', function(d) {
                infoDiv.html(`Min: ${d["Min (MHz)"]} MHz, Max: ${d["Max (MHz)"]} MHz`)
                    .style('opacity', 1);
            })
    ...
    const buttonsContainer = d3.select('.filters');
    buttonsContainer.selectAll('button.filter-button')
      .on('click', function () {
        const region = d3.select('#region-select').property('value');
        const keyword = d3.select(this).attr('data-filter');
        filterByKeyword(region, keyword);
      });
